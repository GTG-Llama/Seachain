...
st.title('Financial News Sentiment Analysis and Query Answering System')

prompt = st.text_input('Input you query here:')
...
article_sample = """
    ...
    Kevin Gallagher, director of Boston University's Global Development Podlicy Center, said the long-delayed U.S. funding came "just in nick of time, given exorbitant interest rates in poorer countries, especially in Africa," that have hit low-income countries hard, compounding already high debt burdens.
    ...
    "I look forward to continuing our partnership with the IMF to support the needs of low-income countries,” Yellen said. (Reporting by Anrea Shalal; Editing by William Mallard)
"""

llm = OpenAI(temperature=0.1, verbose=True, model="gpt-3.5-turbo-instruct")

if prompt and title and article:
    with get_openai_callback() as cb:
        ...
        st.write("The answer to the query based on the article is:")
        st.write(response_1)
        print("response_1:\n", response_1)
        ...
        response_2 = chain_title.run(title=title)
        response_3 = chain_article.run(article=article)
        
        st.write(f"The sentiment of the title is: {response_2}")
        st.write(f"The sentiment of the article is: {response_3}")
